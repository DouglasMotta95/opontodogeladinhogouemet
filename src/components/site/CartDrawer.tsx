    ...
    <Sheet open={isOpen} onOpenChange={setOpen}>
      <SheetContent className="flex w-full flex-col gap-0 p-0 sm:max-w-md">
        ...
        <div className="space-y-3 border-t border-border bg-background p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          ...
          {canCheckout ? <Button asChild size="lg" className="h-12 w-full rounded-2xl text-base font-black"><Link to="/checkout" onClick={() => setOpen(false)}>Finalizar pedido →</Link></Button> : <Button size="lg" className="h-12 w-full rounded-2xl font-black" disabled>{items.length === 0 ? "Adicione itens ao pedido" : missing > 0 ? `Adicione mais ${brl(missing)}` : "Pedidos indisponíveis"}</Button>}
